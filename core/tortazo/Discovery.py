...
class Discovery:
    ...
    def listAuthorityExitNodes(self):
        ...
        log.info(term.format("[+] Retrieving the first %d records in the Descriptors." % (self.cli.exitNodesToAttack),
                             term.Color.GREEN))
    ...
    def recordNmapScan(self, scan, descriptor):
        ...
        for host in scan.all_hosts():
            ...
            if scan[host].has_key('status'):
                ...
                log.info(term.format("[-] There's no match in the Nmap scan with the specified protocol %s" %(protocol), term.Color.RED))
