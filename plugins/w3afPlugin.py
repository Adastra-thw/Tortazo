...
class w3afPlugin(BasePlugin):
    ...
    def getPluginOptions(self, pluginType, pluginName):
        print self.w3afCorePlugin.plugins.get_plugin_options(pluginType,pluginName)

    def setPluginOptions(self, pluginType, pluginName, pluginSetting, pluginValue):
        print self.w3afCorePlugin.plugins.set_plugin_options(pluginType,pluginName)

    def getPluginStatus(self, pluginType, pluginName):
        enabledPlugins = self.w3afCorePlugin.plugins.get_all_enabled_plugins()
        enabled = False
        for type in enabledPlugins.keys():
            if type in pluginType and enabledPlugins[type] in pluginName:
                enabled = True
        if enabled:
    ...
    def listProfiles(self):
        ...
        print "\n"
        tableProfiles = PrettyTable(["Profile File", "Name", "Target", "Description"])
        for profile in valid_profiles:
            tableProfiles.add_row([profile.get_profile_file(),
                                   profile.get_name(),
                                   profile.get_target(),
                                   profile.get_desc()])
        print tableProfiles
    ...
    def executeCommand(self,shellId, command):
        shells = kb.get_all_shells()
        response = None
        for shell in shells:
        ...
        for vuln in kb.get_all_vulns():
            if vuln.get_id() is not None:
                if int(vulnId) in vuln.get_id():
                    shellList = pluginAttack.exploit(vuln.get_id())
    ...
    def help(self):
        ...
        tableHelpPlugins.add_row(['setPluginOptions', 'Set Options for the plugin specified.', 'self.setPluginOptions("audit","blind_sqli","setting","value")'])
        tableHelpPlugins.add_row(['getPluginStatus', 'Check if the specified plugin is enabled.', 'self.getPluginStatus("audit","blind_sqli","setting","value")'])
        ...
        tableHelpMiscSettings.add_row(['setMiscConfig', 'Sets a Misc Settings', 'self.setMiscConfig("setting","value")'])
